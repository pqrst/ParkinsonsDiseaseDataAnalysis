...
#parkinsons <- read.csv("C:\\c\\Rawls\\spring18\\MVA\\project\\parkinsons_updrs.csv", header = TRUE, stringsAsFactors = TRUE)
str(parkinsons)
```
#Check null values
```{r}
...
# adding row and column names in the correlation matrix
colnames(parkcorr) <- row.names(parkcorr) <- parklabs <- c(colnames(parkinsons))
# calculating range of all the variables 
...
Criterion 2 suggests that the first two coordinates are required to represent majority of the data points since the cummulative proportion is 0.78 and close to the threshold value of 0.8. Hence the MDS plot can be on a 2D scatterplot.
...
The MDS plot clearly shows that age is creating a deviation between the datasets with female on the right and male on the left. This significant deviation is because the voice pitch, frequency and amplitude totally differs by being in different ranges for different genders. 
...
2. All Shimmer variables follow a pattern
...
parkinson.EFA <- factanal(parkinsons[, c(2:5,8,16,18:22)], 5, n.obs = nrow(parkinsons), rotation="promax", 
                          scores = "regression")
parkinson.EFA
print(parkinson.EFA$loadings, cut = 0.4)
head(parkinson.EFA$scores)
```
...
library(semPlot)
#cov matrix
parkinsonscov<- cor(parkinsons[,c(2,3,5,8,16,18:22)]) #8,16,19,20,21,22
#model 
parkinson_CFA <- specifyModel(file = "C:\\ttu\\spring18\\MVA\\project\\parkinson_sem_model.txt")

opt <- options(fit.indices = c("GFI", "AGFI", "SRMR"))

parkinson_sem <- sem(parkinson_CFA, parkinsonscov, nrow(parkinsons))
